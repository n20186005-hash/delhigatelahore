...
- `astro build` → `8 page(s) built` (`/`, `/privacy/`, `/terms/`, `/cookies/` and the English twins
  `/en/`, `/en/privacy/`, `/en/terms/`, `/en/cookies/`), then
  `[@astrojs/sitemap] sitemap-index.xml created at dist`.
- `astro check` → **0 errors, 0 warnings, 6 hints** (the pre-existing inline Google Analytics tags
  and the service-worker register script, duplicated across the two language builds).
- `dist/` contains `index.html`, `en/index.html`, `manifest.webmanifest`, `sw.js`, `robots.txt`,
  `sitemap-index.xml`, `sitemap-0.xml`, `images/og-cover.png`,
  `icons/{icon-192,icon-512,maskable-512}.png` and `_astro/*.css`.
- Served from `astro preview` (static server): `/`, `/en/`, `/manifest.webmanifest`, `/sw.js`,
...
- Weather data comes from public endpoints that need no credentials;
  attribution lives in `README.md` and this file, not on the page.

## Multilingual build (Urdu ↔ English)

- `/` → `<html lang="ur" dir="rtl">` with `rel=canonical` → `https://delhigatelahore.com/`;
  `/en/` → `<html lang="en" dir="ltr">` with `rel=canonical` → `https://delhigatelahore.com/en/`.
- Both pages carry the full reciprocal set: `hreflang="ur"`, `hreflang="en"` and `hreflang="x-default"`
  (pointing at the Urdu root), plus `og:locale` / `og:locale:alternate` (`ur_PK` ↔ `en_US`).
- Each homepage carries a `.lang-switch` link at the end of the sticky nav, and every legal page
  cross-links to its counterpart in the other language.
- `sitemap-0.xml` lists all 8 URLs with `xhtml:link rel="alternate" hreflang="ur-PK" | "en-US"`
  alternates (generated by the `i18n` option of `@astrojs/sitemap`).
- In both builds: three valid JSON-LD blocks, **no `aggregateRating`**, one `<h1>`, identical section
  anchors, 7 forecast cards and 10 FAQ entries.
- Shared code between the two versions: `src/data/attraction.ts`, `src/lib/weather.ts` (bilingual
  WMO / AQI / advice dictionaries), `src/components/WeatherRefresh.astro`.
- Visitor-facing text in both languages contains **no provider / pricing / credential notes**
  (no "免费", no "密钥", no "free plan", no key mentions).
